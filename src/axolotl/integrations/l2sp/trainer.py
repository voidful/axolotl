...
"""
Trainer for the L2-SP forgetting baseline.
"""
...
class AxolotlL2SPTrainer(AxolotlTrainer):
    """
    L2-SP trainer with a quadratic anchor to the initialization.
    """
    ...
    def compute_loss(
        ...
        anchor_penalty = quadratic_reference_penalty(
            named_params=dict(iter_named_trainable_params(model)),
            reference_params=self.reference_params,
        )
        loss = ce_loss + ((getattr(self.args, "l2sp_lambda", 1e-4) or 1e-4) * anchor_penalty)
        return (loss, outputs) if return_outputs else loss
